...
# override settings from command line, if available
import sys
if len(sys.argv)>1:
	if len(sys.argv) < 4:
		print("Usage: ReddiWrapTest.py <username> <password> <mod_subreddit> [readonly]")
		sys.exit()
	USERNAME = sys.argv[1]
	PASSWORD = sys.argv[2]
	MOD_SUB = sys.argv[3]

	if len(sys.argv) > 4 and sys.argv[4] and sys.argv[4] not in ('0', 'False'):
		print('*** READ-ONLY MODE *** (many tests will be skipped)')
		READ_ONLY = True

	if not MOD_SUB:
		print('*** NO MODERATOR SUBREDDIT *** (moderator tests will be skipped)')
